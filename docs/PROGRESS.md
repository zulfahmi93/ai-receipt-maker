...
- **T3b.3 SKImage double-dispose nit** in `tests/ReceiptToolkit.Core.Tests/Rendering/Sections/HeaderSectionTests.cs` (lines ~113–123). Test wraps the same `SKImage` in `using` AND hands it to `RenderContext` whose `Dispose` also disposes `ResolvedLogo`. SkiaSharp guards double-dispose internally so the test passes; the ownership story is muddled. Test cleanup follow-up — surface in any future `tests/` pass; do not block sub-clusters B/C/D on it.
- **MetaSection time-row prints `10:42 a` with the sample fixture.** `examples/sample_receipt_data.json` has `options.timeFormat: "hh:mm a"`; per divergence #12 the .NET format spec is `tt`, and lone `a` is emitted literally. `MetaSection` faithfully renders whatever `DateTimeFormatter.FormatTime` returns — the bug surface lives in the fixture or the formatter, not the section. T2c.5 already fixed the test fixture (`timeFormat="hh:mm tt"`); the JSON sample fixture has not been refreshed to match. Cosmetic only (no test asserts on the time row). Resolve at Phase 3b close as part of visual review fixture hygiene OR sooner if a sub-cluster B/C/D test starts asserting on time-row content.

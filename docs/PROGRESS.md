...
Last verified: 2026-05-08 (Phase 4 close — CLI `validate` / `generate` / `sample` sub-commands wired via System.CommandLine 2.0.7; Build 0/0; Cli 7/7; Core 108/108 passed + 2 skipped (Linux-only goldens); Contracts 7/7. CLI process tests spawn `dotnet exec receipt-toolkit.dll` via `Process.Start` and resolve the binary path through an `AssemblyMetadata` attribute set in the test csproj — see divergence #24.).
